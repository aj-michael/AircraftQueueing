...
# Objective
def CalcCustomerHappiness(M):
    return -1*sum(M.pob[i]*(M.tot[i]-M.apt[i]) for i in M.Flight)
M.CustomerHappiness = Objective(rule=CalcCustomerHappiness, sense=maximize)
...
def EnsureFirstObjectiveMet(M):
    return -1*sum(M.Miss[i,j] for i in M.Flight for j in M.Person) >= Soln['Solution'][0]['Objective']['x1']['Value']
M.FirstObjectiveMet = Constraint(rule=EnsureFirstObjectiveMet)
